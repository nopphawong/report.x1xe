...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-12">
                <h1 class="text-center fw-light">Deposit Times</h1>
            </div>
        </div>
    </div>
</section>
                            ...
                            <div class="col-md-8">
                                <div class="d-flex flex-row gap-3">
                                    <div class="d-flex flex-row align-items-baseline">
                                        <label class="me-1" for="end_date" style="width: 190px;">Deposit Times: </label>
                                        <select class="form-select" v-model="form.cnt" @change="search_data">
                    ...
                    <div class="card-body table-responsive">
                        <template v-if="table.filtered.length">
                            <Datatable class="table table-striped" :value="table.filtered" :size="`small`" paginator :rows="rows.perpage">
                                ...
                                <template v-if="form.cnt == `1`">
                                    <Column field="amt_1" header="Deposit Amount" sortable></Column>
                                    <Column field="date_1" header="Deposit Date" sortable>
                                        <template #body="{data}">
                                            {{ formatDate(data.date_1) }}
                                        </template>
                                    </Column>
                                </template>
                                <template v-else-if="form.cnt == `2`">
                                    <Column field="amt_2" header="Deposit Amount" sortable></Column>
                                    <Column field="date_2" header="Deposit Date" sortable>
                                        <template #body="{data}">
                                            {{ formatDate(data.date_1) }}
                                        </template>
                                    </Column>
                                ...
                                <template v-else>
                                    <Column field="amt_3" header="Deposit Amount" sortable></Column>
                                    <Column field="date_3" header="Deposit Date" sortable>
                                        <template #body="{data}">
                                            {{ formatDate(data.date_1) }}
                                        </template>
                                    </Column>
        ...
        methods: {
            async list() {
                ...
                let {
                    ...
                    message,
                    data
                } = await post(`report/registerlistsbycnt`, this.form)
                this.loading = false
            ...
            calculate_deposit() {
                ...
                switch (this.form.cnt) {
                    case `1`:
                        total = data.reduce((pre, cur) => pre + Number(cur.amt_1), 0)
                        break;
                    case `2`:
                        total = data.reduce((pre, cur) => pre + Number(cur.amt_2), 0)
                        break;
                    default:
                        total = data.reduce((pre, cur) => pre + Number(cur.amt_3), 0)
                        break;
                }
                const avg = total / data.length
                this.data.avg = format_number_with_commas(avg.toFixed(2))
